...
function read_dxf(io::IO, ::Type{Roadway};
    ...
    b_LWPOLYLINE = occursin.("LWPOLYLINE", lines)
    b_8 = occursin.("  8", lines)
    b_AcDbPolyline = occursin.("AcDbPolyline", lines)
    b_90 = occursin.(" 90", lines)
    b_10 = occursin.(" 10", lines)
    b_LWPOLYLINE = occursin.("LWPOLYLINE", lines)

    i = findnext(b_LWPOLYLINE, i)
    while i != nothing
        println(i)
        # println(string("<",lines[i],">"))
        # println(b_LWPOLYLINE[i])
        # println(b_8[i])
