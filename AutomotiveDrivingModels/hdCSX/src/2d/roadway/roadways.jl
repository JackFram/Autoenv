...
function _fit_curve(
    ...
    s_arr = collect(range(0.0,stop=L,length=n))
...
function read_dxf(io::IO, ::Type{Roadway};
    ...
    lines = readlines(io)	
    i = findfirst(isequal("ENTITIES"), lines)
    ...
    i = findnext(isequal("LWPOLYLINE"), lines, i)
    while i != nothing
        i = findnext(isequal("  8"), lines, i)
        if i != nothing # segment identifier found in LWPOLYLINE

            if occursin(r"(?<=seg)(\d*)", lines[i+1])
                
                segid = parse(Int, match(r"(?<=seg)(\d*)", lines[i+1]).match)
                i = findnext(isequal("AcDbPolyline"), lines, i)
                
                i != nothing || error("AcDbPolyline not found in LWPOLYLINE!")
                i = findnext(isequal(" 90"), lines, i)
                
                i != nothing || error("Number of vertices not found in AcDbPolyline!")
                ...
                i = findnext(isequal(" 10"), lines, i)
                
                i != nothing || error("First point not found in AcDbPolyline!")
    ...
    for (tag, pts) in lane_pts_dict
        ...
        if !haskey(lane_prev_dict, tag)
            ...
            if best_tag != NULL_LANETAG
                # connect 'em
                #println(pts)
                #poly = ConvexPolygon(pts) # added
                
                #poly.shift!(lane_pts_dict[best_tag][best_ind]) # changed
